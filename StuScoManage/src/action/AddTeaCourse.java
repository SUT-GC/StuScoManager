...
public class AddTeaCourse extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		if(result == 0){
			inputStream = new ByteArrayInputStream("添加失败，该记录已经存在".getBytes("utf-8"));
		}
		if(result == -1){
			inputStream = new ByteArrayInputStream("添加失败 , 教师工号不存在 ".getBytes("utf-8"));
		}
		if(result == -2){
			inputStream = new ByteArrayInputStream("添加失败 , 学生学号不存在 ".getBytes("utf-8"));
		}
		return SUCCESS;
	}
}
